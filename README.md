...
- **一元管理**: 全リージョンの EC2 が参照する設定値（リダイレクト先 URL 等）を、**東京リージョン（ap-northeast-1）** の SSM Parameter Store に「本尊」として集約しています。  
- **疎結合な構成**:   
    - `/00_ssm_base`: 設定（本尊）の管理  
    - `/01_redirect_compute`: リソース（実体）の管理  
- **運用の利点**: ディレクトリを物理的に分離することで、マルチリージョン展開における管理の複雑性を排除。インフラ構成を変更することなく、東京の SSM を書き換えて EC2 を再起動（またはランブック実行）するだけで、世界中の拠点の挙動を制御できる柔軟性を確保しました。  

#### 3. 再起動耐性の実装（Self-Persistence）  
- **Cloud-init の活用**: UserData 内で、実行スクリプト自身を `/var/lib/cloud/scripts/per-boot/` へコピーする処理を実装。  
- **動的同期**: これにより、インスタンスが再起動するたびに「本尊」から最新の設定をプルし直し、常に最新のビジネスロジックが適用される「自己修復・自己更新型」のインスタンスとして動作します。  
